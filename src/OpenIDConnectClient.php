...
class OpenIDConnectClient
{
    ...
    /**
     ...
     private function get_key_for_header($keys, $header)
     {
         ...
         if (isset($header->kid)) {
             throw new Exception('Unable to find a key for (algorithm, kid):' . $header->alg . ', ' . $header->kid . ')');
         } else {
    ...
    {
        if (!class_exists('\phpseclib\Crypt\RSA') && !class_exists('Crypt_RSA')) {
            throw new Exception('Crypt_RSA support unavailable.');
        }
        if (!(property_exists($key, 'n') and property_exists($key, 'e'))) {
            throw new Exception('Malformed key object');
        }

        /* We already have base64url-encoded data, so re-encode it as
           regular base64 and use the XML key format for simplicity.
        */
        $public_key_xml = '<RSAKeyValue>'.PHP_EOL.
            '  <Modulus>' . self::b64url2b64($key->n) . '</Modulus>'.PHP_EOL.
            '  <Exponent>' . self::b64url2b64($key->e) . '</Exponent>'.PHP_EOL.
            '</RSAKeyValue>'.PHP_EOL;
    ...
    private function verifyHMACJWTsignature($hashtype, $key, $payload, $signature)
    {
        if (!function_exists('hash_hmac')) {
            throw new Exception('hash_hmac support unavailable.');
        }
    ...
    case 'HS256':
        ...
        case 'HS384':
            ...
            break;
        default:
            throw new Exception('No support for signature type: ' . $header->alg);
        }
        return $verified;
    }
    ...
    public function register()
    {
        ...
        if (isset($json_response->{'client_secret'})) {
            ...
            throw new Exception('Error registering: Please contact the OpenID Connect provider and obtain a Client ID and Secret directly from them');
        }
    }
     ...
     * @return bool
     */
    private static function hashEquals($signature, $expected)
    {
        if (function_exists('hash_equals')) {
            return hash_equals($signature, $expected);
        }

        $len1=static::safeLength($signature);
        $len2=static::safeLength($expected);

        //compare strings without any early abort...
        $len = min($len1, $len2);
        $status = 0;
        for ($i = 0; $i < $len; $i++) {
            $status |= (ord($signature[$i]) ^ ord($expected[$i]));
        }
